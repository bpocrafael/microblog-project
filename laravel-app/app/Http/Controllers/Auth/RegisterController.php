...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function register(CreateUserRequest $request): RedirectResponse
    {
        ...
        return redirect()->back()->with('success', 'Registration complete. Please check email');
    }
    ...
    public function verifyEmail(string $verificationCode): RedirectResponse
    {
        ...
        if (!$user) {
            return redirect()->route('register')->with('error', 'Verification link expired. Resend another one.');
        }
    ...
    public function resendEmail(ResendEmailRequest $request): RedirectResponse
    {
        ...
        /** @var User $user */
        if ($user == null) {
            return redirect()->route('register')->with('error', 'Email doesn\'t exist in the database.');
        }
        if ($user->email_verified_at) {
            return redirect()->route('register')->with('error', 'Email already verified');
        }
